...
const Classic = () => {
    ...
    const startClassicGame = async () => {
        ...
        sessionStorage.setItem("gameState", JSON.stringify({round: 0, totalRounds: rounds, started: false, locations: response.data.locations, roundTimer: seconds, currTimer: null, currTargetLocation: null, currLocation: null, finishedRound: false, failed: false, results: []}));
        ...
        <div className="flex flex-col items-center font-orbitron w-full h-screen">
            <div className="w-full mb-10">  
                ...
                <div className="inline-block w-1/3 float-left border-box text-center">
                    <h1 className="text-3xl font-orbitron font-bold">Classic Game Mode</h1>
                </div>
            ...
            <p>In this gamemode, you are dropped into a location, and must guess where you are on the mini-map before time's up!</p>
            <div className="flex flex-col w-full h-full items-center justify-end">
                <button className="text-2xl w-32 mb-40 border rounded bg-gray-200" onClick={startClassicGame}>Start</button>
                {err && (<p>{err}</p>)}
